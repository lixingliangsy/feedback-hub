...
export default function handler(req: NextApiRequest, res: NextApiResponse) {
  if (req.method === 'GET') {
    ...
    if (exportFmt === 'json') {
      ...
      res.setHeader('Content-Disposition', 'attachment; filename="feedback-hub-runs.json"')
    ...
    if (exportFmt === 'csv') {
      ...
      const csv = rows
        .map((r) =>
          [r.runId, r.step, r.status, r.createdAt, r.updatedAt, r.rulesetVersion || '']
            .map((v) => `"${String(v).replace(/"/g, '""')}"`)
            .join(','),
        )
        .join('\n')
      res.setHeader('Content-Type', 'text/csv')
      res.setHeader('Content-Disposition', 'attachment; filename="feedback-hub-runs.csv"')
